...
public class Area implements StringSerializable {
    ...
    public double distance(Location loc) {
        double xDif = calcDist(loc.getX(), minX, maxX);
        double yDif = calcDist(loc.getY(), minY, maxY);
        double zDif = calcDist(loc.getZ(), minZ, maxZ);
        return Math.sqrt(xDif * xDif + yDif * yDif + zDif * zDif);
    }

    private double calcDist(double point, int min, int max) {
